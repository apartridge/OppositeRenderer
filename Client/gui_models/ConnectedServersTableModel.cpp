...
QVariant ConnectedServersTableModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::DisplayRole)
    {
        ...
        case 5:
            return QString("%1/sec").arg(
                connection.getNumIterationsReceived() / connection.getTotalTimeSeconds(), 0, 'f', 2);
        case 6:
            return QString("%1/%2")
                .arg(connection.getNumPendingIterations())
                .arg(connection.getPendingIterationsLimit());
        case 7:
        ...
        case 9:
            return connection.getTotalTimeSeconds() > 0 ? QString("%1 MB/s").arg(
                       float(connection.getBytesReceived()) / float(1024 * 1024) / connection.getTotalTimeSeconds(),
                       0,
                       'f',
                       1)
                                                        : "-";
        case 10:
            return QString("%1, %2/sec")
                ...
                .arg(
                    connection.getTotalTimeSeconds() > 0
                        ? connection.getNumPacketsReceived() / float(connection.getTotalTimeSeconds())
                        : 0,
                    0,
                    'f',
                    1);
        ...
        case 12:
            return connection.getNumPacketsReceived() > 0
                ...
                : "-";
        case 13:
            return connection.getNumPacketsReceived() > 0 ? QString("%1 MB/packet")
                                                                .arg(
                                                                    connection.getBytesReceived() / float(1024 * 1024)
                                                                        / float(connection.getNumPacketsReceived()),
                                                                    0,
                                                                    'f',
                                                                    1)
                                                          : "-";
...
QVariant ConnectedServersTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    case 10:
        return "Packets";
    case 11:
        return "Avg Req-Resp time";
    case 12:
        return "Iterations/packet";
    case 13:
        return "MB/packet";
    case 14:
        return "State";
