...
ComputeDeviceInformationWidget::ComputeDeviceInformationWidget(
    QWidget* parent, ComputeDeviceRepository& computeDeviceRepo)
    : QWidget(parent)
{
    setupUi(this);
...
void ComputeDeviceInformationWidget::setupGpuDeviceTabPages(ComputeDeviceRepository& computeDeviceRepo)
{
    ...
    this->setWindowTitle(
        QString("Configure Compute Devices (") + QString::number(computeDevices.size()) + QString(" devices found)"));

    for (int i = 0; i < computeDevices.size(); i++)
    {
        ComputeDeviceInformationWidgetTabPage* deviceTabPage
            = new ComputeDeviceInformationWidgetTabPage(this, computeDevices.at(i));
