...
// If the camera has a aperture > 0, we model it using a thin lens model. Camera.lookdir length is the focal length of
// the camera.
__inline__ __device__ void modifyRayForDepthOfField(
    const Camera& camera, optix::float3& rayOrigin, optix::float3& rayDirection, RandomState& randomState)
{
    if (camera.aperture > 0)
    {
        ...
        float focalPlaneT = (optix::dot(camLookDir, focalPlaneCenterPoint) - optix::dot(camLookDir, camera.eye))
            / optix::dot(camLookDir, rayDirection);
