...
#define OPTIX_DEBUG_PRINT(depth, str, ...)                                                                             \
    if (launchIndex.x == 328 && launchIndex.y == 139)                                                                  \
    {                                                                                                                  \
...
createCoordinateSystem(const optix::float3& N, optix::float3& U, optix::float3& V /*, optix::float3& W*/)
{
...
static __device__ __host__ __inline__ bool
hitFromOutside(const optix::float3& rayDirection, const optix::float3& normal)
{
