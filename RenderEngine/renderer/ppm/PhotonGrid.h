...
__host__ __device__ __inline optix::uint3
getPhotonGridIndex(const optix::float3& position, const optix::float3& sceneWorldOrigo, const float& cellSize)
{
    float3 positivePosition = position - sceneWorldOrigo;
    return floor3f(positivePosition * (1.f / cellSize));
}

__host__ __device__ __inline unsigned int
getPhotonGridIndex1D(const optix::uint3& gridPosition, const optix::uint3& gridSize)
{
    return gridPosition.x + gridPosition.y * gridSize.x + gridPosition.z * gridSize.x * gridSize.y;
}

__host__ __device__ __inline unsigned int
getHashValue(const optix::uint3& gridPosition, const optix::uint3& gridSize, const unsigned int max)
{
