...
getNormalMappedNormal(const float3& normal, const float3& tangent, const float3& bitangent, const float4& normalMap)
{
...
RT_PROGRAM void closestHitPhoton()
{
    ...
    for (int i = 0; i < photonPrd.depth; i++)
        printf("\t");
