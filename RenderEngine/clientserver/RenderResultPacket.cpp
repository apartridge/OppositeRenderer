...
RenderResultPacket::RenderResultPacket(
    unsigned long long sequenceNumber, QVector<unsigned long long> iterationNumbersInPacket, QByteArray output)
...
QDataStream& operator<<(QDataStream& out, const RenderResultPacket& results)
{
    ...
    quint64 sizeIterationNumbersInPacketVector
        = (quint64)(iterationNumbersInPacket.size() * sizeof(unsigned long long) + sizeof(quint32));

    quint64 size = sizeOutputBuffer + sizeIterationNumbersInPacketVector;
    size += (quint64)sizeof(quint64) + 2 * (quint64)sizeof(float);

    out << size << (quint64)results.getSequenceNumber() << iterationNumbersInPacket << results.getRenderTimeSeconds()
        << results.getTotalTimeSeconds() << output;
    return out;
}
