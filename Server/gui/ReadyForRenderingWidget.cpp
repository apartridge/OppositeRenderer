...
void ReadyForRenderingWidget::setRenderTime(float renderTime, float totalTime)
{
    ...
    ui->timeSpentRenderingLabel->setText(
        QString("%1 seconds (%2 %)").arg(renderTime).arg((renderTime / totalTime) * 100, 0, 'f', 1));
}

void ReadyForRenderingWidget::setPendingRenderCommandsAndIterations(
    unsigned int pendingRenderIterations, unsigned int pendingRenderCommands)
{
    ui->pendingRenderIterationsCommandsLabel->setText(
        QString("%1/%2").arg(pendingRenderIterations).arg(pendingRenderCommands));
}